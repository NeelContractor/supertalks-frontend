...
export async function startCheckout(
  ...
  const res = await paymentsApi.initiate(payment.id, returnTo);
  ...
  return { kind: "settled", outcome: await paymentsApi.complete(payment.id) };
}
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      while (Date.now() < deadline) {
        try {
          last = await paymentsApi.get(paymentId);
