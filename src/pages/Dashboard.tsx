...
export default function DashboardPage() {
  ...
  return viewAs === "client" ? <ClientDashboard /> : <AstrologerDashboard />;
}
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Track your bookings, slots and question chats from here.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent>
            {recentQuestions.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Ask a question from any astrologer&apos;s page to get started.
              </p>
            ) : (
              <div className="space-y-3">
                {recentQuestions.map((q) => (
        ...
        <Card>
          ...
          <CardContent>
            {recentBookings.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Book a slot from any astrologer&apos;s page to get started.
              </p>
            ) : (
              <div className="space-y-3">
                {recentBookings.map((b) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium">
                        {b.astrologer?.user.name ?? "Astrologer"}
                      </p>
                      <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                        <Clock className="h-3 w-3" />
                        {new Date(b.startAt).toLocaleString()}
